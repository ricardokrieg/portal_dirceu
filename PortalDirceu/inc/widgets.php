...
class Apex_Team_Ephemera_Widget extends WP_Widget {
	 ...
	 * @since Apex Team 1.0
	 *
	 * @return Apex_Team_Ephemera_Widget
	 */
	public function __construct() {
		parent::__construct( 'widget_apexteam_ephemera', __( 'Apex Team Ephemera', 'apexteam' ), array(
			'classname'   => 'widget_apexteam_ephemera',
			'description' => __( 'Use this widget to list your recent Aside, Quote, Video, Audio, Image, Gallery, and Link posts', 'apexteam' ),
		) );
		...
		$this->format_strings = array(
			'aside'   => __( 'Asides',    'apexteam' ),
			'image'   => __( 'Images',    'apexteam' ),
			'video'   => __( 'Videos',    'apexteam' ),
			'audio'   => __( 'Audio',     'apexteam' ),
			'quote'   => __( 'Quotes',    'apexteam' ),
			'link'    => __( 'Links',     'apexteam' ),
			'gallery' => __( 'Galleries', 'apexteam' ),
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( $ephemera->have_posts() ) :
				...
				<article <?php post_class(); ?>>
					<div class="entry-content">
						...
						<p class="wp-caption-text">
							<?php
								printf( _n( 'This gallery contains <a href="%1$s" rel="bookmark">%2$s photo</a>.', 'This gallery contains <a href="%1$s" rel="bookmark">%2$s photos</a>.', $total_images, 'apexteam' ),
			...
			<a class="post-format-archive-link" href="<?php echo esc_url( get_post_format_link( $format ) ); ?>"><?php printf( __( 'More %s <span class="meta-nav">&rarr;</span>', 'apexteam' ), $this->format_strings[ $format ] ); ?></a>
			<?php
	...
	function form( $instance ) {
		...
		?>
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title:', 'apexteam' ); ?></label>
			<input id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" class="widefat" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>"></p>

			<p><label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php _e( 'Number of posts to show:', 'apexteam' ); ?></label>
			<input id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="text" value="<?php echo esc_attr( $number ); ?>" size="3"></p>

			<p><label for="<?php echo esc_attr( $this->get_field_id( 'format' ) ); ?>"><?php _e( 'Post format to show:', 'apexteam' ); ?></label>
